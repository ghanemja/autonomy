...
def encode(frame, ovstream, output):
    ...
    try:
        ...
        print("Encoding failed: {}".format(err))
    return True
...
def main():
    ...
    try:
        ...
        print("Failed to connect or start video: {}".format(e))
        return
    ...
    # Stream and output file for video
    output = av.open('archive.mp4', 'w')
    ovstream = output.add_stream('mpeg4')
    ...
    for packet in container.demux((video_st,)):
        for frame in packet.decode():
            ...
            if save:
                ...
                if counter > 300:
                    output.close()
                    save = False  # Corrected assignment here

            if key == ord('q') or key == 27:  # Exit on 'q' key or ESC key
                break
